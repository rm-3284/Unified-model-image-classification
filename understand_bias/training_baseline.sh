...
torchrun --nproc_per_node=$NUM_GPUS_TO_USE \
    main.py \
    --data_names "bagel,emu,gemini,gpt,janus,mmada,showo2" --train_dir_name "$TRAIN_DIR" \
    --nb_classes 7 --output_dir "${OUTPUT_ROOT}/baseline/${TRAIN_DIR}" \
    --log_dir "${TENSORBOARD_ROOT}/baseline/${TRAIN_DIR}" \
