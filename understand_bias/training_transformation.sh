#!/bin/bash
#SBATCH --job-name=classifier_train_transformation  # Job name
...
torchrun --nproc_per_node=$NUM_GPUS_TO_USE \
    ...
    --nb_classes 5 --output_dir "${OUTPUT_ROOT}/${TRAIN_DIR}/${TRANSFORMATION}" \
    --log_dir "${TENSORBOARD_ROOT}/${TRAIN_DIR}/${TRANSFORMATION}" \
