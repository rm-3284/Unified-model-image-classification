...
CHKPT="/n/fs/vision-mix/rm4411/understand_bias/checkpoints/${DOMAIN}/train/checkpoint-199.pth"

torchrun --nproc_per_node=$NUM_GPUS_TO_USE --master_port=$MASTER_PORT \
    main.py \
    --data_names "BAGEL,Emu,Janus,MMaDA,show,gpt,gemini" \
    --nb_classes 5 --output_dir "${OUTPUT_ROOT}/${DOMAIN}/${TRAIN_DIR}" \
    --log_dir "${TENSORBOARD_ROOT}/${DOMAIN}/${TRAIN_DIR}" \
    --batch_size 32 --transformation "reference" --domain "$DOMAIN2" \
    --eval True --resume "${CHKPT}"
