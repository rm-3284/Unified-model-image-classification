...
torchrun --nproc_per_node=$NUM_GPUS_TO_USE --master_port=$MASTER_PORT \
    main.py \
    --data_names "BAGEL,Emu,Janus,MMaDA,show,gpt,gemini" --train_dir_name "$TRAIN_DIR" \
    --nb_classes 5 --output_dir "${OUTPUT_ROOT}/${DOMAIN}/${TRAIN_DIR}" \
    --log_dir "${TENSORBOARD_ROOT}/${DOMAIN}" \
    --epochs 200 --lr 1e-3 --warmup_epochs 2 --batch_size 32 \
    --transformation "reference" --domain "$DOMAIN"
