#!/bin/bash
#SBATCH --job-name=classifier_train_corruption  # Job name
...
torchrun --nproc_per_node=$NUM_GPUS_TO_USE \
    main.py \
    --data_names "BAGEL,Emu,Janus,MMaDA,show" --train_dir_name "$TRAIN_DIR" \
    --nb_classes 5 --output_dir "${OUTPUT_ROOT}/${CORRUPTION}/${TRAIN_DIR}" \
    --log_dir "${TENSORBOARD_ROOT}/${CORRUPTION}/${TRAIN_DIR}" \
    --epochs 200 --lr 1e-3 --warmup_epochs 2 --batch_size 128 \
    --transformation "$CORRUPTION" --blur 5

# --transformation "cj" --cj 10
# --transformation "noise" --noise 2.0
# --transformation "blur" --blur 2.0
# --transformation "resize" --shortest_side 16
