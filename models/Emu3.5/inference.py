...
def inference(
    ...
    for i in range(start_idx, end_idx):
        ...
        file_name = os.path.join(save_path, f"{image_id}.png")
        if os.path.exists(file_name):
            print(f"{file_name} already exists at step {i}", flush=True)
            continue
    ...
    #        continue
        ...
        for result_tokens in generate(cfg, model, tokenizer, input_ids, unconditional_ids, full_unc_ids, force_same_image_size):
            try:
                ...
                success = False
                print(f"[ERROR] Failed to generate token sequence: {e}", flush=True)
                break
