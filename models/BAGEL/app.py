...
parser.add_argument("--image-ratio", type=str, choices=["1:1", "4:3", "3:4", "16:9", "9:16"], default="1:1")
...
def text_to_image(prompt, show_thinking=False, cfg_text_scale=4.0, cfg_interval=0.4, 
    ...
    end_time = time.time()
    print(f"image_shape: {image_shapes}, time to generate image {end_time - start_time:.4f}")
...
if __name__ == "__main__": 
    ...
    print(f"read prompts from {args.prompt}")
    whole = len(df)
    start_idx = 0
    end_idx = len(df)
    for i in range(start_idx, end_idx):
        ...
        generated_image, _ = text_to_image(str(prompt), image_ratio=args.image_ratio) # this is type Image.Image
        
        generated_image.save(full_path)
        print(f"image {i} generation finished")
    #prompt = args.prompt
    #generated_image, _ = text_to_image(prompt, image_ratio=args.image_ratio)
    #full_path = os.path.join(output_dir, "test2.png")
    #generated_image.save(full_path)
