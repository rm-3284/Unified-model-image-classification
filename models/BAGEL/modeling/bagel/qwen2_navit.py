...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward_inference(
        ...
        packed_query_sequence = packed_query_sequence.to(device)
        query_lens = query_lens.to(device)
        packed_query_position_ids = packed_query_position_ids.to(device)
        packed_query_indexes = packed_query_indexes.to(device)
        key_values_lens = key_values_lens.to(device) if key_values_lens is not None else None
        packed_key_value_indexes = packed_key_value_indexes.to(device) if packed_key_value_indexes is not None else None
